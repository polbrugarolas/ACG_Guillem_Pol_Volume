...
void main()
{
  ...
  for (float i = ta; i<tb; i += steps) {
    count++;
    if (renderType == 0) {
		  // Compute BT and Optical Thickness T(0, tmax)
		  optical_thicknessT0 = (tb - ta) * absorption_coef;

      // Compute Optical Thickness T(t, tmax) and Absorption Coefficient(t)
      abs_coef_t += (steps) * absorption_coef;
  ...
  BT = bg_color * exp(-optical_thicknessT0);
