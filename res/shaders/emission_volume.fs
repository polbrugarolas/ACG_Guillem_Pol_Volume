...
void main()
{
  ...
  BT = bg_color * exp(-(tb - ta) * absorption_coef);  

	if (renderType == 0) {
    optical_thickness = (tb - ta) * absorption_coef;
    radiance = u_color * exp(-optical_thickness) * absorption_coef + BT;
	}
	else {
		for (float i = ta; i<tb; i += steps) {
			...
			radiance += u_color * exp(-(tb - ta) * n * absorption_coef) * absorption_coef;
		}
